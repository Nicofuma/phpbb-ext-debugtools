 ...
 */
class lint extends \phpbb\console\command\command
{
	/**
...
The command finds all twig templates in the <comment>foo/bar</comment> extension and validates
the syntax of each Twig template. <info>(not supported yet)</info>
...
EOF
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		elseif (is_dir($filename))
		{
			...
			$files = $finder->core_path($filename)->suffix('.html')->get_files();
		}
		else
		{
			// TODO Find all the templates files in one given extension.
			$dir   = $this->getApplication()->getKernel()->locateResource($filename);
			$files = Finder::create()->files()->in($dir)->name('*.html');
		}
